...
AREG_API unsigned int NEMath::crc32Calculate( const std::string & strData )
{
    unsigned int result = static_cast<unsigned int>(~0);   // initialize
    const unsigned int* crc32Tab = reinterpret_cast<const unsigned int *>(::_crc32LookupTable);   // get converted lookup table
    for ( const unsigned char * str = reinterpret_cast<const unsigned char *>(strData.c_str()); *str != static_cast<unsigned char>('\0'); ++ str )
    {
        result = (result >> 8) ^ crc32Tab[(*str) ^ static_cast<unsigned char>(result & 0x000000FF)];  // calculate
    }
...
AREG_API unsigned int NEMath::crc32Calculate( const std::wstring & strData )
{
    unsigned int result = static_cast<unsigned int>(~0);   // initialize
    const unsigned int* crc32Tab = reinterpret_cast<const unsigned int *>(::_crc32LookupTable);   // get converted lookup table
    for ( const unsigned short * str = reinterpret_cast<const unsigned short *>(strData.c_str()); *str != static_cast<unsigned short>('\0'); ++ str )
    {
        unsigned short data  = *str;
        if ( data <= static_cast<unsigned short>(0x00FF) )
        {
            result = (result >> 8) ^ crc32Tab[data ^ static_cast<unsigned char>(result & 0x000000FF)];  // calculate
        }
        else
        {
            unsigned char low    = static_cast<unsigned char>(MACRO_LO_BYTE16( data ));
            unsigned char high   = static_cast<unsigned char>(MACRO_HI_BYTE16( data ));
            result = (result >> 8) ^ crc32Tab[low  ^ static_cast<unsigned char>(result & 0x000000FF)];  // calculate low bits
            result = (result >> 8) ^ crc32Tab[high ^ static_cast<unsigned char>(result & 0x000000FF)];  // calculate hight bits
        }
    }
