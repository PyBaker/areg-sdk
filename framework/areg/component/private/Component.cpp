...
Component* Component::loadComponent(const NERegistry::ComponentEntry &entry, ComponentThread & componentThread )
{
    ...
    if (component != nullptr)
    {
        ...
        for (int i = 0; i < wThreads.getSize(); ++ i)
        {
            ...
            IEWorkerThreadConsumer* consumer = static_cast<Component *>(component)->workerThreadConsumer(wtEntry.mConsumerName, wtEntry.mThreadName);
            if (consumer != nullptr)
            {
                component->createWorkerThread( wtEntry.mThreadName, *consumer, componentThread );
            }
        }
    }
...
void Component::unloadComponent( Component& comItem, const NERegistry::ComponentEntry& entry )
{
    ...
    for (int i = 0; i < wThreads.getSize(); ++ i)
    {
        comItem.deleteWorkerThread( wThreads[i].mThreadName );
    }

    entry.mFuncDelete(comItem, entry);
}

Component* Component::findComponentByName( const std::string & roleName )
{
    ASSERT(roleName.empty() == false);
...
bool Component::existComponent( const std::string & roleName )
{
    ASSERT(roleName.empty() == false);
...
//////////////////////////////////////////////////////////////////////////
Component::Component( ComponentThread & masterThread, const std::string & roleName)
    : RuntimeObject           ( )
...
Component::Component( const std::string & roleName )
    : RuntimeObject           ( )
...
// Methods
//////////////////////////////////////////////////////////////////////////
WorkerThread* Component::createWorkerThread( const std::string & threadName, IEWorkerThreadConsumer& consumer, ComponentThread & /* masterThread */ )
{
    WorkerThread* workThread = mComponentInfo.findWorkerThread(threadName);
    if (workThread == nullptr)
    {
        OUTPUT_DBG("Going to Create WorkerThread object [ %s ]", threadName.c_str());
        workThread = DEBUG_NEW WorkerThread(threadName, self(), consumer);
        if (workThread != nullptr)
        {
            if (workThread->createThread(NECommon::WAIT_INFINITE))
            {
                OUTPUT_DBG("Registering WorkerThread [ %s ]", threadName.c_str());
...
void Component::deleteWorkerThread( const std::string & threadName )
{
    OUTPUT_DBG("Going to Delete WorkerThread object [ %s ]", threadName.c_str());
    WorkerThread* workThread = mComponentInfo.findWorkerThread(threadName);
    if (workThread != nullptr)
    {
        OUTPUT_DBG("Unregistering and deleting WorkerThread [ %s ]", threadName.c_str());
...
void Component::shutdownComponent( ComponentThread& /* comThread */ )
{
    ...
    {
        OUTPUT_INFO("Shutting down worker thread [ %s ] of component [ %s ]", workerThread->getName().getString(), getRoleName().getBuffer());
...
StubBase* Component::findServerByName( const std::string & serviceName )
{
...
IEWorkerThreadConsumer* Component::workerThreadConsumer( const std::string & /* consumerName */, const std::string & /* workerThreadName */)
{
    return nullptr;
}
